...
import { selectCurrentChannel, setCurrentChannelId, DEFAULT_CHANNEL_ID } from '../../store/channelsSlice.js';

const RemoveChannelModal = () => {
  const { t } = useTranslation('modals');
  const dispatch = useDispatch();
  const { channelId } = useSelector(selectModalState);
  const channelToRemove = useSelector(selectCurrentChannel).name;
